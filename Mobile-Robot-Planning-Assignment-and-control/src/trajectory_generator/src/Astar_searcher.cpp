...
double Astarpath::getHeu(MappingNodePtr node1, MappingNodePtr node2) {
  ...
  double tie_breaker = 0.001 * node1->g_score;//use g_score to break
...
bool Astarpath::AstarSearch(Vector3d start_pt, Vector3d end_pt) {
  ...
  while (!Openset.empty()) {
    ...
    currentPtr->id = -1; // searched tag it
    Openset.erase(Openset.begin());//erase it 
    ...
    //3.拓展当前节点
    AstarGetSucc(currentPtr, neighborPtrSets, edgeCostSets);//use astargetsucc to extend
    //4.填写信息，完成更新
    for(unsigned int i=0;i<neighborPtrSets.size();i++)
    {
      ...
      if(neighborPtr->id==0)
      {//changed
        neighborPtr->g_score = tentative_g_score;//new score 
        neighborPtr->f_score = tentative_g_score + getHeu(neighborPtr, endPtr);
        neighborPtr->Father = currentPtr;//set father 
        neighborPtr->id = 1;
        Openset.insert(make_pair(neighborPtr->f_score, neighborPtr));//insert
      }
...
vector<Vector3d> Astarpath::getPath() {
  ...
  for (auto it = front_path.rbegin(); it != front_path.rend(); ++it) {
    Vector3d coord = gridIndex2coord((*it)->index);

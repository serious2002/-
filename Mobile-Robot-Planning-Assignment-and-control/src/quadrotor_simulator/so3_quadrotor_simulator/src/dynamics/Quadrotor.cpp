                      ...
                      Quadrotor::InternalState& dxdt, const double /* t */)
{
  ...
  // // v_dot = //?????
  v_dot = Eigen::Vector3d(0, 0, -g_);  // g_a
  v_dot += (thrust / mass_) * R.col(2); // thrust_z
  v_dot += -resistance * vnorm / mass_;         // air
  v_dot += external_force_ / mass_;     // external
  
  //v_dot = (external_force_ - resistance * vnorm + thrust * R.col(2)) / mass_ - Eigen::Vector3d(0, 0, -g_);
  //v_dot = (thrust * R.col(2) - resistance * vnorm- g_ * Eigen::Vector3d::UnitZ() * mass_ + external_force_ ) / mass_ ;
  acc_ = v_dot;

  R_dot = R * omega_vee;
  //请在这里补充完四旋翼飞机的动力学模型，角速度导数的计算涉及到惯性矩阵J_的逆、力矩、科里奥利力（通过角速度与惯性矩阵和角速度的叉积来计算）和外部力矩等因素。
  // omega_dot = //??????
  Eigen::Vector3d coriolis = J_.inverse() * (moments - (J_ * cur_state.omega).cross(cur_state.omega)); // coriolis
  omega_dot = coriolis + J_.inverse() * external_moment_;  // plus ext
